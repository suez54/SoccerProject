...
public class SpawnManagerX : MonoBehaviour
{
    ...
    void SpawnEnemyWave(int enemiesToSpawn)
    {
        ...
        { // 1. Instantiate the new enemy and store a reference to it.
            var newEnemy = Instantiate(enemyPrefab, GenerateSpawnPosition(), enemyPrefab.transform.rotation);
            // 2. Get the EnemyX component from the new enemy.
            var enemy = newEnemy.GetComponent<EnemyX>();
            // 3. Call the public method on the enemy script to set its speed.
            enemy.IncreaseSpeedByWave(waveCount);
        }
